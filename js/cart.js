...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // create a toast and show it (message string, type)
  function showToast(message = "عملیات انجام شد", type = "success") {
    ...
    toastWrap.className =
      "fixed top-6 inset-x-0 flex justify-center z-50 pointer-events-none";
    toastWrap.innerHTML = `
  ...
  function renderCart() {
    ...
    const forBranch = branch
      ? cart.filter((i) => i.branchId === branch.id)
      : cart;
    ...
    header.innerHTML = `
      <h2 class="text-2xl font-bold mb-1">${
        branch ? escapeHtml(branch.name) : "سبد خرید"
      }</h2>
      <p class="text-sm text-gray-600">${
        branch ? escapeHtml(branch.desc || "") : ""
      }</p>
    `;
    ...
    if (forBranch.length === 0) {
      ...
      forBranch.forEach((item) => {
        ...
        row.className =
          "bg-white bg-opacity-90 rounded-lg p-4 flex justify-between items-center";

        row.innerHTML = `
          <div>
            <div class="font-bold">${escapeHtml(item.name)}</div>
            <div class="text-sm text-gray-600">${formatPrice(item.price)}</div>
            <div class="text-xs text-gray-500 mt-1">از شعبه: ${escapeHtml(
              item.branchName || ""
            )}</div>
          </div>
    ...
    summaryInner.innerHTML = `
      ...
      <div class="border-t border-gray-200 pt-4">
        <button id="submitOrder" class="w-full bg-[#16a34a] hover:bg-[#159644] text-white px-4 py-2 rounded-lg font-bold ${
          itemCount === 0 ? "opacity-60 cursor-not-allowed" : ""
  ...
  function modifyCartItem(name, delta) {
    ...
    const idx = cart.findIndex(
      (i) => i.name === name && i.branchId === branchId
    );
    ...
    try {
      window.dispatchEvent(
        new CustomEvent("cartUpdated", { detail: { branchId } })
      );
    } catch (e) {}
    renderCart();
  }
